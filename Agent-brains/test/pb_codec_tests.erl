...
%% 因此涉及中文的测试数据用 u("中文") 帮助函数构造 UTF-8 binary,
...
%% 帮助: 把字符串字面量转成 UTF-8 binary
-compile([{nowarn_unused_function, u/1}]).
...
llm_infer_request_roundtrip_test() ->
    BizReq = #{kind => llm_infer,
               ...
               api_key => <<"sk-test-key">>,
               messages => [#{role => <<"system">>, content => u("你是一个助手")},
                            #{role => <<"user">>, content => u("北京天气")}],
               tools => [#{name => <<"get_weather">>,
                           description => u("获取城市天气"),
    ...
    ?assertEqual(u("获取城市天气"), maps:get(description, ToolDesc)),
    ok.
...
tool_exec_request_roundtrip_test() ->
    ...
    ?assertEqual(u("{\"city\":\"北京\"}"), maps:get(arguments_json, Inner)),
    ok.
...
llm_infer_response_decode_test() ->
    ...
    ?assertEqual(u("今天北京 25 度"), maps:get(content, BizResp)),
...
llm_infer_response_with_reasoning_test() ->
    Inner = #{content => u("最终回答"),
              tool_calls => [],
              prompt_tokens => 100,
              completion_tokens => 80,
              reasoning_content => u("正在思考天气查询...")},
    AgentResp = #{llm_infer => Inner},
    Bin = hermes:encode_msg(AgentResp, 'AgentResponse'),

    BizResp = pb_codec:decode_resp(Bin),
    ?assertEqual(u("正在思考天气查询..."), maps:get(reasoning_content, BizResp)),
    ?assertEqual(u("最终回答"), maps:get(content, BizResp)),
    ok.
...
llm_infer_response_with_tool_calls_test() ->
    ...
    ?assertEqual(u("{\"city\":\"上海\"}"), maps:get(arguments, TC)),
    ok.
...
tool_exec_response_success_test() ->
    Inner = #{result_json => u("{\"temp\":25,\"city\":\"北京\"}"),
              error => <<>>},
    ...
    ?assertEqual(u("{\"temp\":25,\"city\":\"北京\"}"), maps:get(result_json, BizResp)),
...
end_to_end_roundtrip_test() ->
    ...
    GoRespBin = hermes:encode_msg(#{tool_exec => #{result_json => u("{\"temp\":30}"),
                                                    error => <<>>}}, 'AgentResponse'),

    %% 4. Erlang 侧解出业务响应
    BizResp = pb_codec:decode_resp(GoRespBin),
    ?assertEqual(tool_exec, maps:get(kind, BizResp)),
    ?assertEqual(u("{\"temp\":30}"), maps:get(result_json, BizResp)),
    ok.
